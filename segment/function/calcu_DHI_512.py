...
def calcu_HV(L1_calcu_HV):
    ...
    # # 将当前椎体的坐标点添加到列表中
    # vertebrae_points.append(corners_st)
    with open('corners_st.pickle', "ab") as file:
        pickle.dump(corners_st, file)
    # 第二种
    ...
    HV = np.sum(src) / L_wid
    print('HV, point_L_h, point_L_w',HV, point_L_h, point_L_w)
...
def calcu_DHI(output):
    ...
    point_big = [point_L1_h, point_L1_w, point_L2_h, point_L2_w, point_L3_h, point_L3_w, point_L4_h, point_L4_w, point_L5_h, point_L5_w, point_S1_h, point_S1_w]
    print(point_big)
    arr = []
    for i in range(6):
        data = []
        for j in range(4):
            data.append([point_big[i*2][j],point_big[i*2+1][j]])
        arr.append(data)
    print(arr)
    with open('H_W.pickle', "wb") as file:
